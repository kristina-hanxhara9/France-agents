---
mode: agent
description: 'Enrich every row in france_retailers-with-keywords.xlsx with live web data (website, phone, email, socials, channel) and write both .md and .json outputs.'
...
2. For **every** row, use `#websearch` to search the live internet with the row's `search_query` column (or build `{company_name} {trade_name} france` if that column is missing)
...
- Score `web_channel_guess` against keywords found in the search/fetch text:
...
- Work in batches of 10 rows. **Append** each batch's output to both files before moving on, so progress is never lost if the chat is interrupted.
